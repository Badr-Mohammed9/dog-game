export function createDino(k, dinoSprite, type = "green") {
  if (type === "green") {
    return [
...
export function dinoMovement(k, dino, blockWidth, NUM_BLOCKS) {
  ...
  if (dino.front && dino.pos.x < maxBlock) {
    ...
    moveDino(k, dino, dino.pos.x + blockWidth);
  } else {
    dino.front = false;
  }
...
function moveDino(k, dino, x) {
  return k.tween(dino.pos, k.vec2(x,dino.pos.y), 3, (val) => (dino.pos = val));
}
